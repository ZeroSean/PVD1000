...
//线性拟合矫正曲线
static const double line_k[4] = {0.9729, 0.9998, 0.9950, 0.9660};
static const double line_d[4] = {0.2487, 0.1604, 0.2017, 0.2729};
int calculate_rangefromTOF(int idx, int32 tofi) {
	...
	if(idx < 3 && idx >= 0) {
		//使用线性拟合函数矫正距离值
		distance = line_k[idx] * distance + line_d[idx]; 
	}
	if(distance < 0) {
		distance = 0;
	}
	if(distance > 20000) {
		return 0;					// discard any results >20 km
	}
 ...
 * */
static const double rangeChannelBias[4] = {0.35, 0.35, 0.25, 0.25};
int instance_calculate_rangefromTOF(int idx, uint32 tofx)
{
	...
	if(inst->smartPowerEn)
	{
		...
		if(inst->configData.chan == 5 || inst->configData.chan == 4)
		{
	...
	} else if(inst->configData.chan == 4) {
		distance_to_correct = distance/1.51;
	} else
	{
...
//		if(inst->configData.chan == 4) {
//			if(distance > (2 * rangeChannelBias[idx])) {
//				distance -= rangeChannelBias[idx];
//			} else if(distance > rangeChannelBias[idx]) {
//				distance = distance * 0.8;
//			}
//		}
	if(inst->configData.chan == 4) {
		if(distance > 5.0) distance = 1.0295 * distance;				// (通道4，64M，110k)
...
//查看instance是否正忙，若是的话，其它耗时任务最好先不要执行，以免干扰测距协议
//0:空闲， 非0：忙
uint8 instance_is_busy(uint32 reqTimeMs) {
	instance_data_t *inst = &instance_data[0];
	if(inst->mode == TAG) {
		if(TA_INIT == inst->testAppState || inst->testAppState == TA_RX_WAIT_DATA) {
			return 0;
		} if(((inst->testAppState == TA_SLEEP_DONE) && (inst->instanceTimerEn == 1))) {
			//若标签在预计所要花耗费的时间内唤醒
			if((portGetTickCnt_10us() + reqTimeMs * 100 - inst->instanceSleepTime_us) >= inst->nextWakeUpTime_us) {
				return 1;
			} else {
				return 0;
			}
