...
{ 
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) 	//读取接收中断标志位USART_IT_RXNE 													
	{
		...
		pc_dat.next++;
		if(pc_dat.next == MAX_RECDATA_SIZE) {
			pc_dat.next = 0;
		}
				
		if((REC_DATA_ST & 0x80) == 0) {
			if(ReceiveDate == '#') {//结束符
				REC_DATA_ST |= 0x80;	
			}		
		}
...
void dataParse() {
	int state = 1;
	if(pc_dat.pre == pc_dat.next) {
		return;
	}
	if((REC_DATA_ST&0x80) == 0) {
		return;
	}
	for(; pc_dat.pre < pc_dat.next; pc_dat.pre++) {
		switch(pc_dat.buf[pc_dat.pre]) {
			case 'S':
			case 's':
				if((pc_dat.next - pc_dat.pre) >= SET_ANC_POSITION_SIZE) {
					state = updateAncPosition((void*)&pc_dat.buf[pc_dat.pre]);
					if(state == 0) {
						pc_dat.pre = 0;
						pc_dat.next = 0;
						REC_DATA_ST = 0;
			...
			case 'E':
			case 'e':
				if((pc_dat.next - pc_dat.pre) >= (1+3)) {
					state = setElectronicThreshold((void*)&pc_dat.buf[pc_dat.pre]);
					if(state == 0) {
						pc_dat.pre = 0;
						pc_dat.next = 0;
						REC_DATA_ST = 0;
