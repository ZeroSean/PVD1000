...
void DWM_Init(void) {
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_32;//4--9Mhz, 2--18MHz(36MHz Baud Rate Prescaler)(DW1000 SPI 20MHz Max)
...
void DWM_RSTnIRQ_setup(int enable) {
	...
	if(enable) {
		...
		GPIO_EXTILineConfig(DW_RESET_PortSource, DW_RESET_PinSource);
		...
		NVIC_InitStructure.NVIC_IRQChannel = EXTI0_IRQn;		//pin #0 -> EXTI #0
...
in current implementation it enables all IRQ from line5:9
*/
