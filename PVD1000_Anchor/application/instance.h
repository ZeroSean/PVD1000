...
#define SIDOF		1	//session if offset
#define FLGOF		2	//flags offset
#define CSNOF		3	//cluster slot number offset
#define CFNOF		4	//cluster frame number offset
#define CMPOF		5	//cluster map offset
...
#define GP_UPPOF	3	//group poll update period offset or electric field value
...
#define CORRECT_RANGE_BIAS  (0)     // Compensate for small bias due to uneven accumulator growth at close up high power
...
typedef struct {
	...
	uint8   lastRecBcnNum;		//前一次接收到的bcn在beacon slot中的号码
}BCNMan;
...
typedef struct {
	...
	uint8 posValid;		//坐标有效性
	uint32 pos[3];			//x,y,z坐标，单位mm
...
/*******************anchor变量*******************************/
	...
	uint16  resetSampleNum;		//进行网络统计状态复位的采集周期数
